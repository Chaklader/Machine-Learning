...
setwd("u:\\")
mfund.data<- read.table(file = "L:\\LS_Verkehrstechnik\\AppliedStatistics\\ExLab2_MfundData\\mfund.data", header = T)
library(MASS)
attach(mfund.data)  #always detach the dataframe after finishing your work
options(width = 70, digits = 2) #sets global options
mfund.data[1:10, ]
...
par(mfrow = c(1, 1))
plot(ecdf(load)) #Empirical Cumulative Distribution Function
