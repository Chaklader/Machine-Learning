...
#Data sets in the directory ExLab3_DataInputPreparation\\:
#MiD2008_PUF_Personen.dat: original text data from the MID2008 CD, delivered by the Clearingstelle Verkehr
#p2008original.Rdata: original rdata-file from MiD2008_PUF_Personen.dat
#p2008.Rdata: our working rdata-file
...
#loading and preparation of MID2008 personal data for Applied Statistics in Transport 
...
#--------------------------------------------------------------
#Personal Data
p2008original <- read.table("L:\\LS_Verkehrstechnik\\AppliedStatistics\\ExLab3_DataInputPreparation\\MiD2008_PUF_Personen.dat", header=T,sep="\t")
head(p2008original)
...
save(p2008original, file="p2008original.Rdata")
load("p2008original.Rdata")

#Create your data file for the analyses
p2008<-p2008original #change the name
save(p2008, file="p2008.Rdata")

rm(p2008original)
ls()  #"p2008"
str(p2008)

isna<-apply(apply(p2008,2,is.na),2,sum)    #Number of NAs for all variables
table(isna)  #or with only one command apply(apply(wmuc2002,2,is.na),2,sum)
#   0
#  124 #no NA
table(complete.cases(p2008)) #TRUE 60713

#insert NA:
with(p2008, table(hp_alter)) #994: 1,  997: 87,  998: 21,  999: 3
barplot(table(p2008$hp_alter))
summary(p2008$hp_alter)
#   Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#   0.00   23.00   47.00   45.33   61.00  999.00 
hp_alter <- replace(p2008$hp_alter, p2008$hp_alter == "994"
    | p2008$hp_alter == "997"
    | p2008$hp_alter == "998"
    | p2008$hp_alter == "999", NA)
summary(hp_alter)
#   Min. 1st Qu.  Median    Mean 3rd Qu.    Max.    NA's 
#   0.00   23.00   47.00   43.57   61.00  101.00  112.00 
mean(hp_alter) #[1] NA
#What to do?
...
levels(p2008$p01_1) <-c("Jederzeit","Gelegentlich","GarNicht","KeinFührerschein","Verweigert","WeißNicht","102","103","104","105","107","109","110")  #
...
save(p2008, file="ExLab3_DataInputPreparation\\p2008.Rdata")


#-------------
#-------------
#####students task: change the following variables into factor variables: 
...
#should be only valid for hp_beruf=1 ("ja"), check remaining sample size
...
#p0414_3: Accessibility of school by public transportation 
#hp_beruf: Are you currently employed? (Entry from personal/household interview)
...
#-------------
#Exploratory analyses, compute new variables
#for commands look at the mfund.data example, AS_DescriptiveStatistics_mfundData.r 
#For examples with MID2008 see ..\ExLab5_MID2008_Descriptive\MID2008_AppliedStatistics_Descriptive_20111122.R  
